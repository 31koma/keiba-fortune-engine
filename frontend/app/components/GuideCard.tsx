...
// Rタブの金の点=隠れ推しがいるレース / 出馬表の✦=レース内の合トップ。
// ※ 2026-08-15に入れた「騎↓調↓セルを除外」は run12 のP8判定で撤去した。
// ※ 同日、画面を「場タブ → Rタブ → 表」(RaceBoard)へ作り替えた。
//    一覧に出す数字は 合・収・理・調・騎 の5つだけ。主・客・本・数は詳細シートへ。
...
const RULES: [string, string][] = [
  ...
  ["くわしく見たい時は4つの色", "合のとなりに並ぶ 収・理・調・騎。数字は読まなくていい。色は 赤→橙→青→緑→金 の順に良くなります。"],
...
const STARS: [string, string][] = [
  ...
  ["Rタブの金の点", "「隠れ推し」がいるレースの印です。集合意識の注目はまだ薄いのに、収束の強い馬がいることを示します。"],
  ["出馬表の ✦", "そのレースで「合」がいちばん高い馬に付きます。その行は金色で光ります。"],
];
