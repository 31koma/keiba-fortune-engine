...
- [ ] 「今日のシンクロ度」の正式な算出規則を知識ベース側で定義する。
      現状はフロントのMock値(frontend/lib/reading.ts)であり、勝率・的中率・レース結果予測を表すものではない。
      定義項目: 4者(馬・騎手・レース日・利用者)の調和度の構成要素と重み/10点尺度への正規化/
      5段階ラベル文言/色帯域。status=app_hypothesisとして出典・検証計画とともに正本へ収録し、
      確定後にフロントをバックエンドAPI出力へ差し替える
